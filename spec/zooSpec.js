...
describe('#addAnimal', function(){
  ...
  it('should add an animal to the animals array', function(){
      zoo.open();
      zoo.addAnimal(pig = new Animal("Babe",10,"Pig"));
      expect(zoo.animals).toEqual([pig]);
  });

  it('should only add instances of animals', function(){
    zoo.open();
    expect(zoo.addAnimal(bear = new Zoo("Yogi", 80, "bear"))).toEqual("Not an instance of animal");
  });

  it('should not add duplicates', function(){
      zoo.open();
      zoo.addAnimal(bear = new Animal("Yogi", 80, "bear"));
      expect(zoo.addAnimal(bear = new Animal("Yogi", 80, "bear"))).toEqual("Sorry, that animal already lives here");
...
describe('#removeAnimal', function(){
  it('should remove an animal from the animals array if the zoo is open', function(){
    zoo.open();
    expect(zoo.removeAnimal(bear)).toEqual("I think you made me kill it =(");
  });
});
});
